...
@section('content')
    ...
    <div class="container">
        ...
        <div class="main-div">
            ...
            <div class="row mt-3">
                <h5 class="p-3 bg-secondary text-white">Customer Details</h5>
                <div class="col-6">
                    <table class="w-100">
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>{{ $customer_details[0]->CustomerCity }},
                                &nbsp;{{ $customer_details[0]->CustomerPostCode }}
                            </td>
                        </tr>
                    </table>
                </div>
            ...
            <div class="row mt-3">
                <h5 class="p-3 bg-secondary text-white">Flight Details</h5>
            ...
            <div class="row mt-3">
                <h5 class="p-3 bg-secondary text-white">PNR Details</h5>
            ...
            <div class="row mt-3">
                <table class="table table-hover">
                    <thead>
                    <tr class=" p-3 bg-secondary text-white text-center">
                        <td><strong>SR #</strong></td>
                        <td><strong>Passenger Name</strong></td>
                        <td><strong>Type</strong></td>
                        <td><strong>Seat Qty</strong></td>
                    ...
                    @foreach ($passanger_details as $key => $value)
                        ...
                        <tr class="text-center fw-bold">
                            ...
                            <td>{{ $value->PassengerType }}</td>
                            <td>{{ number_format($value->SeatQty,1) }}</td>
                            <td>{{ number_format($value->SeatPrice,1) }}</td>
                            <td>{{ number_format($line_total,1) }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
            ...
            @php
                ...
                $paid = \Illuminate\Support\Facades\DB::table('recipt_details')->where('AgentID',auth()->id())->where('InvoiceNo',$customer_details[0]->InvoiceNo)->get();

                foreach ($paid as $value){
                    $paid_amount += $value->BankAmount + $value->CardAmount + $value->CashAmount + $value->OtherAmount;
                    }
            @endphp
                ...
                <div class="col-12">
                    ...
                    <table class="table bg-light">
                        ...
                        <tr>
                            ...
                            <td class="float-end fw-bold">{{ number_format($total,1) }}</td>
                        </tr>
                        <tr>
                            <th>Total Booking Fee</th>
                            <td class="float-end fw-bold">{{ number_format($booking_fee,1) }}</td>
                        </tr>
                        <tr>
                            <th>Sub Total</th>
                            <td class="float-end fw-bold">{{ number_format($total + $booking_fee,1) }}</td>
                        </tr>
                        <tr>
                            <th>Total Amount Paid</th>
                            <td class="float-end fw-bold">{{ number_format($paid_amount,1) }}</td>
                        </tr>
                        <tr>
                            <th>Remaining Amount</th>
                            <td class="float-end fw-bold">{{ number_format(($total  + $booking_fee) - $paid_amount,1) }}</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="container">
                ...
                <h5 class="text-start">DEPOSITS FOR HOLIDAY ARE NEITHER REFUNDABLE NOR CHANGEABLE (Terms &amp;
                    Conditions
                    May Apply).</h5>
                <p>Unless Specified, All the deposits paid for flights and accommodation purchased/issued is
                    non-refundable.
                    In case of cancellation or no show (Failure to arrive
                    or check-in on time) and non-changeable before or after departure (date change is not permitted).
                    Once
                    holiday is reserved, bookings/tickets are non-transferable to any other person means that name
                    changes
                    are not permitted.</p>
                <p>If you are reserving the flight or accommodation or both by making the advance partial payment
                    (Initial
                    deposit) then please note that fare/taxes may
                    increase at any time without the prior notice. It means the price is not guaranteed unless the
                    ticket is
                    issued because the airline/consolidator has the right to
                    increase the price due to any reason. In that case, we will not be liable and the passenger has to
                    pay
                    the fare/tax difference. We always recommend you to
                    pay ASAP and get issue your flight tickets or holiday to avoid this situation. Furthermore, if you
                    will
                ...
                <p>It is your responsibility to check all the details are correct i.e. Passenger names (are same as
                    ...
                    Origin &amp; Destination, Stop Over, Baggage Allowance, and other flight information. Once the
                    ticket is
                    issued then no changes can be made, unless specified.</p>
                <h5 class="text-start">PASSPORT, VISA &amp; IMMIGRATION REQUIREMENTS</h5>
                <p>You are responsible for checking all these items like Passport, Visa (including Transit Visa), and
                    other
                    ...
                    cannot accept the liability of any transit visa and if you are refused to board the flight or could
                    not
                    clear the immigration or any kind of failure in providing the information required like passport,
                    visa
                    or other documents required by any airline, authority or country. We also recommend you that to
                    check
                    this link <a
                ...
                <p>It is the traveler's responsibility to RECONFIRM your flights at least 72 hours before your departure
                    time either with your travel agent or the relevant Airline directly. The company will not be liable
                    for
                ...
                <p>If you have any special requests like meal preference, Seat Allocation and wheelchair request, etc,
                    please advise us at the time of issuance of the ticket. We will try our best to fulfill these
                    bypassing
                    this request to the relevant airlines, but we cannot guarantee and failure to meet any special
                    request
                    will not hold us liable for any claim.</p>

                <h5 class="text-start">Covid-19</h5>
                <p>Due to Covid-19, Additional entry requirements have been introduced which varies from country to
                    country
                    and may be subject to change with short notice. You are responsible for checking and conforming with
                    the
                ...
                <p>If the flight is affected due to Covid-19 then airline policies will be applied. To accommodate the
                    ...
                    option instead of a refund. In that case, you must have to follow the airline rules and cannot
                    demand a
                    refund. If a full refund is permitted, A admin fee (Per person) will be deducted as the service
                    charges
                    and a refund can take up to 3 months. If the flight is operated by the airline and you decide not to
                    board the flight, then you will be ineligible for a refund. In this case (if your ticket is
                    refundable)
                ...
                <p>Zistravels does not accept responsibility for any kind of loss if the airline fails to operate due to
                    any
                    unforeseen circumstances like weather, war, natural disaster, pandemic, riots, strikes, etc.
                    Passengers
                ...
                <p>We advise you to read our complete terms and conditions mentioned at <a
                        href="http://www.zistravels.co.uk/terms.php">
                        http://www.zistravels.co.uk/terms-and-conditions</a>
                    on our website.</p>
            </div>
        </div>
    </div>
